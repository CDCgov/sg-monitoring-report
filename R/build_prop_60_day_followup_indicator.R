...
#' follow-up for the current rolling 3-month window: Returns results in long format with one row per country.
#'
#' @details
...
#' \describe{
#'   \item{data}{A long-format data frame with one row per country
#'     containing: \code{ctry}, \code{whoregion}, \code{window},
#'     \code{current_period},
#'     \code{current_inad_cases},
#'     \code{current_got_60day},
#'     \code{current_prop_60day},
...
build_prop_60_day_follow_up <- function(afp_data) {
  ...
  eligibility_cutoff <- Sys.Date() - lubridate::days(120)
  ...
  recent_start <- lubridate::floor_date(recent_end %m-% lubridate::period(2, "months"), unit = "month")
  ...
  earlier_start <- lubridate::floor_date(earlier_end %m-% lubridate::period(2, "months"), unit = "month")
  ...
  # Use recent_start as start date required parameter in functions
  ...
  summarize_window <- function(data, start, end, period_label) {
    data |>
  ...
  #Period Counts -----
    # Count follow-up completions for each of the two time periods
  recent_counts  <- summarize_window(eligible_cases, recent_start,  recent_end,  recent_period_label)
  earlier_counts <- summarize_window(eligible_cases, earlier_start, earlier_end, earlier_period_label)

  # Create Full Grid of all Countries + Region -----
    full_grid <- tidyr::expand_grid(
      ctry   = unique(afp_data$place.admin.0),
      period = c(recent_period_label, earlier_period_label)
    ) |>
      dplyr::mutate(whoregion = sirfunctions::get_region(ctry))

    # Bind both windows and join onto grid -----

  all_counts <- dplyr::bind_rows(recent_counts, earlier_counts)

  final_summary <- full_grid |>
    dplyr::left_join(all_counts, by = c("ctry", "period")) |>
    dplyr::mutate(
      ...
      period = factor(period, levels = c(earlier_period_label, recent_period_label))
    ) |>
    dplyr::arrange(ctry, period) |>
    dplyr::select(
      ctry, whoregion, period, period_start, period_end,
      inad_cases, got_60day, prop_60day, flag
    )
  ...
  meta <- list(
    ...
    possible_statuses = c("Within Target", "Off Target", "Incomplete Data", "Review")
  )
