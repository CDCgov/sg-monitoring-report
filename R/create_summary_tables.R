...
#  makes the data for non-monthly indicators that use a "Window" comparison
make_window_plot_data <- function(data, group_col, group_col_new_name, flag_label) {
  data |>
    dplyr::select(dplyr::all_of(group_col), window, current_period, prior_period, current_median_days, prior_median_days) |>
    dplyr::rename(!!group_col_new_name := dplyr::all_of(group_col)) |>
    dplyr::mutate(Flag = flag_label) |>
    dplyr::select(dplyr::all_of(group_col_new_name), window, current_period, prior_period, current_median_days, prior_median_days, Flag)
}
