...
#'
#' Calculates the median number of days from receipt in the sequencing lab to
#' sequencing results across two rolling 3-month windows: the most recent
#' completed 3-month period and the immediately preceding 3-month period. Each
#' window is compared to the same 3-month period in the prior year. Returns
#' results in long format with one row per sequencing lab per window.
#'
#' @details
#' Timeliness is measured as
#' \code{DateofSequencing - DateIsolateRcvdForSeq}, matching the
#' \code{days.seq.rec.res} interval from \code{get_lab_intervals()}. Samples are
#' assigned to windows by \code{DateofSequencing}, so the reporting period
#' reflects completed sequencing results. Intervals outside 0-365 days are
...
build_timeliness_of_sequencing_results_indicator <- function(lab_data, end_date = Sys.Date()) {
  ...
  lab_prep <- lab_data |>
    ...
    dplyr::filter(
      !is.na(seq.lab),
      seq.lab != "",
      tolower(seq.lab) != "unknown",
      !is.na(DateofSequencing),
      !is.na(DateIsolateRcvdForSeq),
      !is.na(days.seq.rec.res),
      dplyr::between(days.seq.rec.res, 0, 365)
    )
  ...
  recent_summary <- build_period(labs, recent_counts, recent_prior_counts) |>
    dplyr::mutate(
      window = "recent",
  ...
  earlier_summary <- build_period(labs, earlier_counts, earlier_prior_counts) |>
    dplyr::mutate(
      window = "earlier",
  ...
  meta <- list(
    ...
    definition = "Median number of days between receipt in sequencing lab and sequencing result. Within Target if the median number of days for the three-month period is within +/-50% compared with the same three-month period of the previous year. Below Target if the median is more than 50% higher than the same period of the previous year.",
    above_target_definition = "Above Target if the median is more than 50% lower than the same period of the previous year.",
    other_status_definitions = "No sequenced samples if there are no current or prior sequenced samples. No current sequenced samples if there are no current samples but prior samples exist. No prior sequenced samples if current samples exist but no prior samples exist.",
