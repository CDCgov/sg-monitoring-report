#' Build WPV/VDPV Notification Timeliness Indicator (With 60-Day Lag)
#'
...
#' most recent 3-month window and the immediately preceding 3-month window.
#' Each window is compared to the same 3-month window in the prior year.
...
#' field are included. Windows are defined by \code{dateonset} with a 60-day eligibility
#' lag — cases must have onset at least 60 days before \code{end_date} to be included,
#' as WPV/VDPV notification to HQ is expected to take a minimum of 30 days from onset.
#' The 60-day cutoff is a conservative buffer to ensure the cohort of cases in each window is
#' reasonably complete. Cases with a \code{days_to_notification} outside the range 0–365
#' are excluded as likely data quality issues. Cases with a missing notification date
#' are also excluded.
#'
...
#' @param end_date Date to anchor the eligibility cutoff. Defaults to
#'   \code{Sys.Date()}. The period will end on the last complete month
#'   at least 60 days prior to this date.
#'
...
#' @examples
#' \dontrun{
#' result <- build_wpv_vdpv_timeliness_indicator_lag(raw_data$pos)
#' result$data
#' result$metadata$recent_period_label
#' }
#'
#' @export
build_wpv_vdpv_timeliness_indicator_lag <- function(pos, end_date = Sys.Date()) {
  ...
  # Cases must have onset >= 60 days ago for notification to reasonably be expected
  eligibility_cutoff <- end_date - 60L

  # Recent 3-month window — last complete month where all cases are eligible
  recent_end <- lubridate::floor_date(eligibility_cutoff, unit = "month") %m-% lubridate::days(1)
  ...
  eligibility_note <- paste0(
    "Eligible window is based on a 60-day notification lag. ",
    "Cases with onset after ", format(eligibility_cutoff, "%b %d, %Y"),
    " are excluded as notification may not yet be complete. ",
    "Analysis dates are set to the last full month prior to ",
    format(eligibility_cutoff, "%b %d, %Y"), "."
  )

  # Prepare Data -----
  pos_prep <- pos |>
    dplyr::filter(source == "AFP") |>
    dplyr::mutate(ctry = place.admin.0,
    ...
    dplyr::filter(dateonset <= recent_end,         # onset-based window anchor
  ...
  summarize_window <- function(data, start, end) {
    data |>
      dplyr::filter(dplyr::between(dateonset, start, end)) |>  # onset-based
      dplyr::group_by(ctry) |>
      dplyr::summarize(
        counts = dplyr::n(),
  ...
  build_period <- function(full_grid, current, prior) {
    full_grid |>
      ...
      dplyr::rename(current_count = counts, current_median_days = median_days) |>
      dplyr::left_join(prior, by = "ctry") |>
      dplyr::rename(prior_count = counts, prior_median_days = median_days) |>
      dplyr::mutate(
  ...
  # Return -----
  meta <- list(
    indicator_code           = "wpv_vdpv_timeliness_lag",
    indicator_label          = "Timely AFP WPV/VDPV Detection (60-day lag)",
