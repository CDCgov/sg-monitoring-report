...
lab_data <- clean_lab_data(lab_data, "2019-01-01", Sys.Date(), afp_data = raw_data$afp)
...
afp_wpv_vdpv <- get_wpv_vdpv_timeliness(raw_data$pos, temporal_scale = "quarter") |>
  sirfunctions:::add_risk_category(ctry_col = "country") |>
  dplyr::select(-Region) |>
  dplyr::relocate(dplyr::any_of("SG Priority Level"), .after = "country")
...
prop_classified <- get_prop_case_classified(raw_data$afp) |>
  ...
  dplyr::select(-Region) |>
  dplyr::relocate(dplyr::any_of("SG Priority Level"), .after = "ctry")
prop_inad <- get_prop_inad_with_contacts(human_specimen, raw_data$afp) |>
  sirfunctions:::add_risk_category(ctry_col = "ctry") |>
  dplyr::select(-Region) |>
  dplyr::relocate(dplyr::any_of("SG Priority Level"), .after = "ctry")
``` 
